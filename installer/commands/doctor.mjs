...
export async function doctor(opts) {
  ...
  for (const [ip, meta] of Object.entries(manifest.files ?? {})) {
    ...
    if (current !== meta.sha256) {
      if (meta.mode === 'config') warnings.push(`config tuned since install: ${ip} (expected — verify the change was human-approved)`)
      else warnings.push(`drift on harness-owned file: ${ip} (run \`update\` to reconcile, or restore it)`)
    }
  }
  ...
  for (const probe of ['CLAUDE.md', '.github/CODEOWNERS']) {
    try {
